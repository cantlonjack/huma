...
export async function GET(request: Request): Promise<Response> {
  // Blocker 4: morning-sheet fans out to /api/sheet and /api/insight via
  // fetch (NOT via the Anthropic SDK directly). The observability-coverage
  // meta-test's INDIRECT_ALLOWLIST forces this wrap; here we actually apply
  // it. source:"cron" keeps per-operator analytics dashboards clean.
    ...
    async (obs) => {
      ...
      for (const userId of userIds) {
        try {
          ...
          // ─── Per-operator observability log (parent_req_id correlation) ─
          // DOCUMENTED GAP: prompt_tokens/output_tokens stay 0 here because
          // /api/sheet and /api/insight don't expose response.usage in their
          // JSON bodies. Children routes log their own usage via Plan 05b's
          // wraps. Phase 2 may surface usage on the JSON response so this
          // cron-level log can include accurate token counts.
