...
// ─── Nudge Generation ─────────────────────────────────────────────────────
// Generates 0-2 proactive nudges using Haiku. Fast, cheap, conservative.
// Zero nudges is better than annoying nudges.

const NUDGE_PROMPT = `You are HUMA's proactive intelligence for {name}.
...
export async function POST(request: Request) {
  ...
  // Build history summary (streaks, skips)
...
function summarizeHistory(history: Array<{ date: string; behaviorKey: string; checked: boolean }>): string {
