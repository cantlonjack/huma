...
interface AccumulatedContext {
  ...
  // Phase 3: Field Reading (generic — not just agriculture)
  field: {
    location: string;
...
### Lotus Flow & Workspace Entities

```typescript
// ─── Operator Context (Lotus Flow output) ───
// Created when the operator completes the Lotus Flow onboarding.
// Replaces ShapeAssessment as the entry data model.
interface OperatorContext {
  ...
  updatedAt: Date;
  version: number;
  lotusProgress: Record<string, boolean>; // which petals complete
}

// ─── Shape Data (legacy + workspace) ───
// Used for workspace snapshots and historical data.
// Each shape is a point-in-time snapshot. Never overwrite — always insert.
interface ShapeData {
  dimensions: {
    body: number;      // 1-5 (internal: Living) — legacy; 1-10 for new pulse
  ...
  createdAt: Date;
  source: 'lotus' | 'workspace' | 'conversation';
}
...
type CapitalKey =
  | 'financial' | 'material' | 'living' | 'social'
  | 'experiential' | 'intellectual' | 'spiritual' | 'cultural';
...
// ─── Saved Shape (Supabase row) ───
interface SavedShape {
  id: string;                          // UUID
  user_id: string;
  dimensions: Record<DimensionKey, number>;
  source: 'lotus' | 'workspace' | 'conversation';
  insight: ShapeInsight | null;        // AI-generated insight
  created_at: string;                  // ISO timestamp
}

// ─── Shape Insight ───
// Generated by Claude API after Lotus Flow synthesis or context edit.
interface ShapeInsight {
  headline: string;                    // One sentence, under 15 words
  detail: string;                      // 2-3 sentences on pattern, coupling, hidden asset
  oneThing: string;                    // Concrete suggestion, under 40 words
  dimensions: {
    highlighted: DimensionKey[];       // 2 dimensions most central to the insight
    lever: DimensionKey;               // Which dimension would cascade most if moved
  };
}

// ─── Context Edit (Workspace) ───
// Created when the operator edits any piece of context in the workspace.
// Capital values update through workspace editing, not daily rituals.
interface ContextEdit {
  id: string;
  userId: string;
  field: string;                       // Which context field was edited
  previousValue: unknown;              // Value before edit
  newValue: unknown;                   // Value after edit
  source: 'workspace' | 'petal' | 'review';  // Where the edit originated
  reason?: string;                     // Optional: why the operator changed it
  createdAt: Date;
}

// ─── DailyPulseMetadata (DEPRECATED) ───
// Retained for backward compatibility with existing data.
// New context updates use ContextEdit instead.
interface DailyPulseMetadata {
  changed: CapitalKey[];
  insightGenerated?: string;
}

// ─── Context Pattern ───
// Detected automatically from workspace edit history.
interface ContextPattern {
  id: string;
  userId: string;
  type: 'coupling' | 'temporal' | 'trend';
  description: string;                 // "Body and Money move together"
  dimensions: DimensionKey[];          // Which dimensions are involved
  confidence: number;                  // 0-1, based on data points
  detectedAt: Date;
  dataPoints: number;                  // How many edits contributed
}
```

**Lotus Flow → Conversation bridge:** When the operator enters the Map Conversation (6-phase Design process), their accumulated Lotus Flow context + workspace edits are injected into the `AccumulatedContext.essence` and `AccumulatedContext.field` structures. The Lotus Flow and context petals build context BEFORE the conversation; the conversation deepens it.
...
-- Operator Context (Lotus Flow output — replaces Shape Builder entry data)
CREATE TABLE operator_contexts (
  ...
  stage TEXT NOT NULL,
  governance JSONB NOT NULL,             -- {solo: bool, people: [{name, relationship}]}
  capitals JSONB NOT NULL,               -- {financial: 7, material: 5, ...} (1-10 each)
  archetype TEXT,
  strengths TEXT[],                       -- top 3 capital names
  growth_areas TEXT[],                    -- bottom 2 capital names
  location TEXT,
  lotus_progress JSONB DEFAULT '{}',     -- {whole: true, who: true, what: true}
  version INTEGER DEFAULT 1,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ
);

-- Shapes (point-in-time snapshots — Lotus Flow, Workspace Edits, Conversation)
CREATE TABLE shapes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id),
  dimensions JSONB NOT NULL,            -- {body: 4, people: 3, ...}
  source TEXT NOT NULL,                  -- 'lotus', 'pulse', 'conversation'
  insight JSONB,                         -- ShapeInsight
  created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE INDEX idx_shapes_user_source ON shapes(user_id, source);
```
...
1. **Lotus Flow** — The 11-screen context painting (entity type, governance, capital spectrum, archetype)
2. **Context Petals** — Ikigai, Purpose, Vision, Behavior (progressive deepening)
3. **Workspace Edits** — Any context element is editable at any time; capital values update through direct editing, not daily rituals
4. **Conversation** — The 6-phase Design conversation and weekly reviews
5. **Community Wisdom** — Anonymized patterns from operators in similar contexts
...
### Lotus Flow Synthesis & First Insight Generation

**Trigger:** Operator reaches Screen 9 of the Lotus Flow (synthesis).
...
```json
{
  ...
  "entityType": "person",
  "stage": "starting",
  "governance": { "solo": false, "people": [{ "name": "Sarah", "relationship": "partner" }] },
  "capitals": {
    "financial": 3, "material": 5, "living": 7, "social": 8,
    "experiential": 4, "intellectual": 6, "spiritual": 7, "cultural": 5
  }
}
```

**Synthesis prompt requirements (Screen 9):**
- Infer archetype from capital pattern (Earth Tender, Maker, Healer, Builder, Seeker, etc.)
- Infer development stage (starting, transition, building, searching)
- Identify top 3 strengths and bottom 2 growth areas
- Voice: direct, no hedging, no flattery. State what you see.
- Maximum: 4 lines (archetype, stage, strengths, growth areas)

**First insight prompt requirements (Screen 12):**
- Voice: fence-post neighbor (see Voice Bible §01-§03)
- Compile a specific pattern recommendation from the RPPL library
- Match pattern to operator's archetype, stage, and capital shape
- Maximum: 3 sentences (connection, first move, invitation)
- Never use banned phrases (see Voice Bible §02)
- Never reference the framework, RPPL, or capital names

**Output format (JSON):**
```typescript
{
  archetype: string;           // e.g. "Earth Tender"
  stage: string;               // e.g. "starting"
  strengths: string[];         // top 3 capital names (user-facing)
  growthAreas: string[];       // bottom 2 capital names (user-facing)
  insight: {
    connection: string;        // 1-2 sentences naming the tension
    firstMove: string;         // Compiled pattern recommendation
    invitation: string;        // "Come back tomorrow — or keep going now."
  }
}
```

**Legacy:** The original Shape Builder insight prompt (`app/src/engine/shape-insight.ts`) is deprecated but retained for existing users who completed the old 8-card flow.

---

## 05 — Beachhead MVP: Application Routes

_Canonical definition in HUMA_PRODUCT_SURFACE.md §19. Reproduced here for technical reference._
...
**Auth boundaries:**
- `/` and `/begin` — unauthenticated. Lotus Flow works without login.
- `/begin` → "Save your progress" triggers auth (magic link) after pattern recommendation, redirect to `/home`
- `/home` — requires auth. Redirects to `/begin` if no auth.
- `/map/[id]` — public, read-only. No auth required.

**API routes:**

```
POST /api/chat              Conversation endpoint
POST /api/generate-map      Map generation
GET  /api/map/[id]          Map data
POST /api/lotus-synthesis   Lotus Flow archetype + insight generation
POST /api/review            Weekly review (v1.5)
```
...
│   │   ├── FieldLayers.tsx           # Field reading layer strip
...
The primary entry point is the Lotus Flow at `/begin` — a 90-second, 11-screen context painting that moves through the first three lotus petals (WHOLE → WHO → WHAT). It captures entity type, life stage, governance, 8-capital spectrum, and produces an archetype synthesis and first pattern recommendation. The Lotus Flow is the operator's first experience with HUMA and must feel immediate, visual, and low-commitment.

The full 6-phase conversation (below) is Layer 5 in the progressive depth model — earned by engagement over days/weeks, not required on day one.
...
- [ ] Lotus Flow (11-screen context painting at /begin, replaces Shape Builder)
