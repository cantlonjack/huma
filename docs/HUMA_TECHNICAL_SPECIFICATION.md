...
### Lotus Flow & Daily Capital Pulse Entities
...
// ─── Shape Data (legacy + pulse) ───
// Still used for daily pulse snapshots and historical data.
...
interface ShapeData {
  ...
  createdAt: Date;
  source: 'lotus' | 'pulse' | 'conversation';
}
...
// ─── Shape Insight ───
// Generated by Claude API after Lotus Flow synthesis or daily pulse.
...
// ─── Daily Capital Pulse (Phase 2) ───
// Created each day the operator checks in (Layer 2, Days 2-14).
// Capital sliders from Lotus Flow become the pulse mechanism.
interface DailyPulseMetadata {
  changed: CapitalKey[];               // Which capitals the operator adjusted
  insightGenerated?: string;           // Pattern detection insight (after day 3-4)
}

// ─── Pulse Pattern (Phase 2) ───
// Detected automatically from pulse history.
interface PulsePattern {
  ...
  detectedAt: Date;
  dataPoints: number;                  // How many pulses contributed
}
```

**Lotus Flow → Conversation bridge:** When the operator reaches week 2-3 and enters the Map Conversation (6-phase Design process), their accumulated Lotus Flow context + pulse data is injected into the `AccumulatedContext.essence` and `AccumulatedContext.field` structures. The Lotus Flow and Daily Capital Pulse build context BEFORE the conversation; the conversation deepens it.
...
-- Shapes (point-in-time snapshots — Lotus Flow, Daily Capital Pulse, Conversation)
CREATE TABLE shapes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id),
...
2. **Capital Pulse** — Daily capital slider check-ins (8 capitals, 15-30 seconds)
3. **Context Petals** — Ikigai, Purpose, Vision, Behavior (progressive deepening)
...
/home           Three-tab experience (requires auth, default: Your Day)
  #pulse        Daily Capital Pulse (returning users)
