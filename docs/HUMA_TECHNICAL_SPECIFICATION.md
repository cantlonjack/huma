...
### Shape Builder & Daily Pulse Entities

```typescript
// ─── Shape Data ───
// Created when the operator completes the Shape Builder, daily pulse, or conversation.
// Each shape is a point-in-time snapshot. Never overwrite — always insert.
// Matches: app/src/types/shape.ts, app/src/lib/shapes.ts
...
// Matches: app/src/lib/shapes.ts
interface SavedShape {
  id: string;                          // UUID
  user_id: string;
  dimensions: Record<DimensionKey, number>;
  source: 'builder' | 'pulse' | 'conversation';
  insight: ShapeInsight | null;        // AI-generated insight (builder only)
  created_at: string;                  // ISO timestamp
}

// ─── Shape Insight ───
// Generated by Claude API when operator completes the Shape Builder.
// Matches: app/src/engine/shape-insight.ts
...
**Shape → Conversation bridge:** When the operator reaches week 2-3 and enters the Map Conversation (6-phase Design process), their accumulated shape + pulse data is injected into the `AccumulatedContext.essence` and `AccumulatedContext.field` structures. The Shape Builder and Daily Pulse build context BEFORE the conversation; the conversation deepens it.
...
-- Shapes (point-in-time snapshots — Shape Builder, Daily Pulse, Conversation)
CREATE TABLE shapes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id),
  dimensions JSONB NOT NULL,            -- {body: 4, people: 3, ...} (1-5 each)
  source TEXT NOT NULL,                  -- 'builder', 'pulse', 'conversation'
  insight JSONB,                         -- ShapeInsight (builder shapes only)
  created_at TIMESTAMPTZ DEFAULT now()
);
...
### Shape Builder Insight Generation

**Trigger:** Operator completes all 8 cards in the Shape Builder.

**Input to AI:**
```json
{
  "scores": {
    "body": 4, "people": 3, "money": 2, "home": 4,
    "growth": 4, "joy": 1, "purpose": 5, "identity": 4
  }
}
```
...
- Analyze the PATTERN of scores, not individual scores
- Address four aspects in flowing prose: unusual aspect, strongest coupling, hidden asset, one lever
- Include a "Try this" suggestion: one concrete, doable action for today
- Maximum length: `headline` under 15 words, `detail` under 60 words, `oneThing` under 40 words
- Never use banned phrases (see Voice Bible §02)
- Never reference the framework, RPPL, or capital names
- Use operator-facing dimension names (Body, People, Money, etc.)

**Output format (JSON):**
```typescript
{
  headline: string;        // One sentence naming the key structural insight
  detail: string;          // 2-3 sentences: unusual divergence, hidden asset, coupling
  oneThing: string;        // Concrete suggestion with coupling rationale
  dimensions: {
    highlighted: DimensionKey[];  // 2 dimensions most central to insight
    lever: DimensionKey;          // Which dimension would cascade most
  }
}
```

The `headline` and `detail` are displayed together as flowing prose on the reveal screen. The `oneThing` is displayed separately as the "Try this" suggestion. The `dimensions` metadata drives UI highlighting on the shape radar.

**Implementation:** `app/src/engine/prompts/shape-insight-prompt.ts` (prompt), `app/src/engine/shape-insight.ts` (parser), `app/src/app/api/shape-insight/route.ts` (API route).
...
/begin          Shape Builder entry (8 cards → insight → fork)
...
**Auth boundaries:**
- `/` and `/begin` — unauthenticated. Shape Builder works without login.
- `/begin` → "Save my shape" path triggers auth (magic link) before redirect to `/home`
...
POST /api/shape-insight     Shape Builder insight generation
